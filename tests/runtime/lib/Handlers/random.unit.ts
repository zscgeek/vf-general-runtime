...
describe('randomHandler unit tests', () => {
  ...
  describe('handle', () => {
    it('no nextIds', async () => {
      ...
      expect(runtime.trace.debug.args).to.eql([['no random paths connected - exiting', Node.NodeType.RANDOM]]);
    });
    ...
    it('1 nextIds', async () => {
      ...
      expect(runtime.trace.debug.args).to.eql([['going down random path', Node.NodeType.RANDOM]]);
    });
