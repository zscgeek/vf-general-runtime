...
describe('dialog manager utilities unit tests', () => {
  ...
  describe('sampleUtterance', () => {
    it('not found', async () => {
      expect(utils.sampleUtterance([], {} as any)).to.eql('');
    });

    it('no slots', async () => {
      expect(utils.sampleUtterance([{}] as any, {} as any)).to.eql('');
    });
    ...
    it('not every', async () => {
      ...
      expect(utils.sampleUtterance(utterances as any, model as any)).to.eql('');
    });

    it('works', async () => {
      const replaceSlotsStub = sinon.stub(utils, 'replaceSlots').returns(' output ');
      ...
      expect(utils.sampleUtterance(utterances as any, model as any)).to.eql('output');
      expect(replaceSlotsStub.args).to.eql([[utterances[1].text, { slot2: 'barcellona' }]]);
    });
  });

  describe('generateButtonsForUtterances', () => {
    it('no utterances', async () => {
      expect(utils.generateButtonsForUtterances([] as any, null as any)).to.eql([]);
    });
    ...
    it('works', async () => {
      ...
      expect(utils.generateButtonsForUtterances(utterances as any, model as any)).to.eql([
        { name: 'name1', request: { type: 'text', payload: 'name1' } },
      ]);
