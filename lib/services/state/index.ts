...
import { CompiledCMSVariable } from '@voiceflow/dtos';
import { parseCMSVariableDefaultValue, VariableDeclaration } from '@voiceflow/utils-designer';
...
class StateManager extends AbstractManager<{ utils: typeof utils }> implements InitContextHandler {
  ...
  initializeFromCMSVariables(variables: Record<string, CompiledCMSVariable>) {
    return Object.fromEntries(
      Object.entries(variables).map(([name, declare]) => [
        name,
        parseCMSVariableDefaultValue(declare as VariableDeclaration) ?? 0,
      ])
    );
  }
